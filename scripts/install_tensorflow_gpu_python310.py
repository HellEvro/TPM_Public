...
# -*- coding: utf-8 -*-
"""
Установка TensorFlow с GPU поддержкой через Python 3.10
Создает venv с Python 3.10 и устанавливает TensorFlow GPU
"""
...
def main():
    print("="*60)
    print("УСТАНОВКА TENSORFLOW GPU ЧЕРЕЗ PYTHON 3.10")
    print("="*60)
    
    # Проверяем Python 3.10
    print("\n[STEP 1] Проверка Python 3.10...")
    result = subprocess.run(['py', '-3.10', '--version'], capture_output=True, text=True)
    if result.returncode != 0:
        print("[ERROR] Python 3.10 не найден!")
        print("Установите Python 3.10: https://www.python.org/downloads/release/python-3100/")
        return False
    
    print(f"[OK] Python 3.10 найден: {result.stdout.strip()}")
    
    # Создаем venv с Python 3.10
    project_root = Path(__file__).resolve().parents[1]
    venv_path = project_root / '.venv_gpu'
    
    print(f"\n[STEP 2] Создание venv с Python 3.10...")
    if venv_path.exists():
        print("[INFO] .venv_gpu уже существует, удаляю...")
        import shutil
        shutil.rmtree(venv_path)
    
    if not run(['py', '-3.10', '-m', 'venv', str(venv_path)], "Создание venv"):
        return False
