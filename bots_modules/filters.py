...
def _check_loss_reentry_protection_static(symbol, candles, loss_reentry_count, loss_reentry_candles, individual_settings=None):
    ...
    try:
        ...
        # ✅ КРИТИЧНО ИСПРАВЛЕНО: Проверяем ПОСЛЕДНЮЮ сделку (самую новую)
        # Логика фильтра: если ПОСЛЕДНЯЯ сделка была убыточной - блокируем
        # НО: если среди последних сделок есть БОЛЕЕ НОВАЯ прибыльная - НЕ блокируем
        # 
        # Проблема была: проверялись первые N сделок, но не учитывалась более новая прибыльная
        # Решение: проверяем только ПОСЛЕДНЮЮ (самую новую) сделку - она уже первая в списке
        all_losses = True
        trade_pnls = []
        ...
        logger.info(f"[LOSS_REENTRY_{symbol}] ✅ КРИТИЧНО: Проверяем ТОЛЬКО ПОСЛЕДНЮЮ (самую новую) сделку - она первая в списке")
        
        # ✅ КРИТИЧНО: Проверяем ТОЛЬКО самую последнюю сделку (первую в списке после сортировки DESC)
        if closed_trades:
            # Берем самую последнюю сделку (первую в отсортированном списке)
            last_trade = closed_trades[0]
            
            for idx, trade in enumerate([last_trade]):  # Проверяем только одну сделку
            pnl = trade.get('pnl', 0)
