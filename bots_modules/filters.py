...
def _check_loss_reentry_protection_static(symbol, candles, loss_reentry_count, loss_reentry_candles, individual_settings=None):
    ...
    try:
        ...
        # ✅ КРИТИЧНО ИСПРАВЛЕНО: Логика проверки в зависимости от N
        # Если N=1: проверяем только последнюю сделку
        # Если N>1: проверяем последние N сделок подряд - блокируем ТОЛЬКО если ВСЕ N сделок убыточные
        
        # Берем последние N сделок для проверки
        trades_to_check = all_closed_trades[:loss_reentry_count] if all_closed_trades else []
        
        # Если нет закрытых сделок - разрешаем вход
        if not trades_to_check or len(trades_to_check) < loss_reentry_count:
            return {
                'allowed': True,
                'reason': f'Недостаточно закрытых сделок ({len(trades_to_check) if trades_to_check else 0} < {loss_reentry_count})',
                'candles_passed': None
            }
        ...
        if loss_reentry_count == 1:
            logger.info(f"[LOSS_REENTRY_{symbol}] ✅ N=1: Проверяем ТОЛЬКО последнюю (самую новую) сделку")
        else:
            logger.info(f"[LOSS_REENTRY_{symbol}] ✅ N={loss_reentry_count}: Проверяем последние {loss_reentry_count} сделок ПОДРЯД (блокируем только если ВСЕ убыточные)")
        
        # Проверяем последние N сделок (они уже отсортированы DESC - самые новые первыми)
        for idx, trade in enumerate(trades_to_check):
            pnl = trade.get('pnl', 0)
        ...
        logger.info(f"[LOSS_REENTRY_{symbol}] 📊 ИТОГО проверки последней сделки: all_losses={all_losses}")
        logger.debug(f"[LOSS_REENTRY_{symbol}] Детали: {', '.join(trade_pnls) if trade_pnls else 'нет данных'}")
        
        # ✅ КРИТИЧНО: Если последняя сделка НЕ в минус - РАЗРЕШАЕМ вход (фильтр НЕ работает)
        if not all_losses:
            logger.info(f"[LOSS_REENTRY_{symbol}] ✅✅✅ ФИЛЬТР НЕ БЛОКИРУЕТ: последняя сделка прибыльная, разрешаем вход")
            return {
                'allowed': True,
                'reason': f'Последняя сделка прибыльная (PnL >= 0), фильтр не применяется',
                'candles_passed': None
            }
        
        logger.info(f"[LOSS_REENTRY_{symbol}] ⚠️ ПОСЛЕДНЯЯ сделка убыточная, проверяем количество свечей...")
        
        # Последняя сделка в минус - проверяем количество прошедших свечей
        # last_trade уже определен выше как all_closed_trades[0]
