...
def _check_loss_reentry_protection_static(symbol, candles, loss_reentry_count, loss_reentry_candles, individual_settings=None):
    ...
    try:
        ...
        logger.debug(f"[LOSS_REENTRY_{symbol}] Детали сделок: {', '.join(trade_pnls) if trade_pnls else 'нет данных'}")
        
        # ✅ КРИТИЧНО: Если НЕ ВСЕ последние N сделок в минус - РАЗРЕШАЕМ вход (фильтр НЕ работает)
        if not all_losses:
            logger.info(f"[LOSS_REENTRY_{symbol}] ✅✅✅ ФИЛЬТР НЕ БЛОКИРУЕТ: не все последние {loss_reentry_count} сделок в минус, разрешаем вход")
            return {
                'allowed': True,
