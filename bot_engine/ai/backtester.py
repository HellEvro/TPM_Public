...
class BacktestEngine:
    ...
    def _open_position(
        ...
        logger.debug(f"[BACKTEST] ОТКРЫТИЕ: {symbol} {side} @ {entry_price:.2f}")
    ...
    def _close_position(
        ...
        logger.debug(
            f"[BACKTEST] ЗАКРЫТИЕ: {position['symbol']} {position['side']} "
            f"@ {exit_price:.2f} | PnL: {pnl_pct:+.2f}% ({net_pnl:+.2f} USDT) | {reason}"
        )
