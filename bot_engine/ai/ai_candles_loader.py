                ...
                try:
                    ...
                    if exchange_type == 'bybit':
                        ...
                        try:
                            ...
                            # При инкрементальном обновлении: загружаем только новые свечи (до 1000)
                            if incremental_mode:
                            ...
                            while (max_requests is None or request_count < max_requests) and len(all_candles) < MAX_CANDLES_TO_LOAD:
                                try:
                                    ...
                                    if response and response.get('retCode') == 0:
                                        ...
                                        # УЛУЧШЕНИЕ: Ограничиваем до MAX_CANDLES_TO_LOAD
                                        ...
                                        if len(all_candles) >= MAX_CANDLES_TO_LOAD:
                                            logger.info(f"   ✅ {symbol}: достигнут лимит загрузки ({MAX_CANDLES_TO_LOAD} свечей)")
                                            break
                            ...
                            if existing_candles_list and all_candles:
                                ...
                                if len(all_candles) > MAX_CANDLES_TO_LOAD:
                                    ...
                                    logger.debug(f"   📊 {symbol}: после объединения ограничено до {MAX_CANDLES_TO_LOAD} последних (самых новых) свечей")
